...
\begin{exerciceAP}~
  ...
  \item Soit $U$ une variable aléatoire réelle de la loi uniforme sur
    ...
    \begin{scilab}
      & \tcFun{function} \tcVar{x} = simulX(\tcVar{n}) \nl %
      & \quad \tcVar{x} = zeros(\tcVar{n},1) \nl %
      & \quad \tcFor{for} i = 1:\tcVar{n} \nl %
      & \quad \quad \tcVar{x}(i,1) = ........ \nl %
      & \quad \tcFor{end} \nl %
      & \tcFun{endfunction}
    \end{scilab}
    \end{noliste}
...
\begin{exerciceSP}~\\
 On considère deux variables aléatoires $X$ et $Y$ telles que : $\E(X) 
 = \dfrac{7}{3}$ et $\E(Y) = \dfrac{14}{3}$.
 \begin{noliste}{1.}
   \item Les graphiques ci-dessous représentent les densités des deux
   \var $X$ et $Y$.\\
   \begin{minipage}{.45\linewidth}
    \begin{center}
    %% French babel fout la merde : ne pas oublier shorthandoff
    \shorthandoff{;}
    \begin{tikzpicture}[xscale=1.5, yscale = 1.5, scale = .75, 
    transform shape, %
      declare function = { %
        f(\x) = 0; %
        g(\x) = (1/12)*\x; %
      ...
      \begin{axis}[%
        ...
        \addplot[samples=2, domain=-1:0, blue, thick] {f(x)}; %
        \addplot[samples=2, domain=0:3, 
        blue, thick]{g(x)}; %
        \addplot[samples=2, domain=3:8, blue, thick] {h(x)}; %
        \addplot[samples=2, domain=8:9, blue, thick] {f(x)}; %
      \end{axis}
    \end{tikzpicture}
    \end{center}
   \end{minipage}
   ...
   \begin{minipage}{.48\linewidth}
    ...
    transform shape, %
      ...
      \begin{axis}[%
        ...
        \addplot[samples=2, domain=0:3, 
        blue, thick]{g(x)}; %
        \addplot[samples=2, domain=3:4, blue, thick] {h(x)}; %
        \addplot[samples=2, domain=4:8, blue, thick] {f(x)}; %
      \end{axis}
    \end{tikzpicture}
    \end{center}
   \end{minipage}
