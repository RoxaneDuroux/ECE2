...
%         \]
      ...
      \end{center}~\\[-1.2cm]
%       \begin{remark}%~%
%         L'énoncé ne demandait pas explicitement le tableau de
%         variations de 
%         complet.
%         On pouvait donc se contenter de répondre que la fonction $u$
%         est strictement croissante sur $]0,+\infty[$ (et ne pas
%         effectuer de calculs de limites).
%       \end{remark}~\\[-1.2cm]
    \end{proof}
  ...
  \begin{remark}
    ...
    \item Démontrons par récurrence que : $\forall n\in\N$, $\PP{n}$,
      ...
      \item {\bf Hérédité} : soit $n\in\N$.\\
        ...
        D'après la question \itbf{9.b)}, $u_{n+1}\geq
        \dfrac{6-\ee}{2} u_n$.\\
...
\begin{remarkL}{.95}%~
  ...
  % Par exemple, \og $1$ \fg{} pourra désigner, suivant son
  % utilisation, le {\bf réel} $1$ ou le {\bf polynôme} constant égal
  % à $1$.
\end{remarkL}
  ...
  \begin{proof}~
    ...
    \begin{remarkL}{.98}%~%
      ...
      \item L'énoncé fournit explicitement l'endomorphisme $g$. Dans
        ...
        b \circ g = \id_E \qquad \text{ et } \qquad g \circ b = \id_E
        \]
        % On procède de même pour les matrices. Si une question demande
        % de démontrer qu'une matrice $A \in \M{n}$ est inversible et
        % que son inverse est la matrice $B \in \M{n}$, il suffit de
        % démontrer :
        % \[
        % AB \ = \ BA \ = \ I_n
        % \]
...
%     \end{scilab}        
    \begin{remark}
      ...
      \item Pour être en accord avec le nombre de lignes utilisées
        dans le programme on pouvait remplacer les lignes \ligne{10} et
        \ligne{11} par :
        \begin{scilabC}{9}
          & \qquad r = r + 1; \tcVar{s} = \tcVar{s} + 1 \nl %          
        \end{scilabC}
...
%       égale à $\E(S_{10})$.
    ...
    \begin{remarkL}{.97}
      ...
      Ceci semble être le cas puisque la question \itbf{9.} demande
...
\item
  ...
  \item En déduire la loi de $X_2$.
    ...
    \begin{proof}~
      ...
      \begin{remark}
        Cela correspond à sommer les colonnes du tableau
  ...
  \item Les variables $X_1$ et $X_2$ sont-elles indépendantes ?
    ...
    \begin{proof}~\\
      ...
      \conc{On en déduit que les \var $X_1$ et $X_2$ ne sont pas
        indépendantes.}
      \begin{remark}
        Ce résultat semble logique : le résultat du premier tirage
        influe sur le contenu de l'urne (ajout d'une boule bleue ou
        rouge) avant le deuxième tirage.
      \end{remark}~\\[-1.4cm]
    \end{proof}
  \end{noliste}
    ...
    \begin{proof}~\\
      ...
      \begin{remark}
        ...
        \item Il faut enfin noter que la formule donnée dans ce
          ...
          si $k = 0$, d'après la question \itbf{4.} :
          \[
