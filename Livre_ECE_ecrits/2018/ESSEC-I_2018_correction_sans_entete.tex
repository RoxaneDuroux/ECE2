\chapter*{ESSEC-I 2018 : le corrigé}
  ...
  \begin{proof}~
    ...
    \begin{remark}
      \begin{noliste}{$\sbullet$}
        ...
        \begin{scilabC}{8}
          & \quad \quad \tcIf{if} X(k) > \tcVar{y} \nl %
	  & \quad \quad \quad \tcVar{y} = X(k) ; \tcVar{z} = \tcVar{y}
        \end{scilabC}
...
%     \end{center}~\\[-2.2cm]
  ...
  \begin{noliste}{a)}
	...
	\conc{$\E(Z_n) \ = \ \dfrac{n(n-1) \alpha \, \lambda^2}
      ...
      \begin{remark}
        Dans les questions \itbf{4.b)}, \itbf{5.a)(iii)} et 
	\itbf{5.c)}, on utilise la
	...
	concernées.\\
        ...
        positives (comme détaillé en question \itbf{1.c)}).
      \end{remark}~\\[-1.4cm]
    \end{proof}
  \end{noliste}
\end{noliste}
...
\begin{noliste}{1.}
  ...
  \begin{noliste}{a)}
	...
	\begin{scilab}
	  ...
	  & \quad V = zeros(1,N) \nl %
	  & \quad \tcFor{for} k = 1:N \nl %
	  & \quad \quad Z(k) = simulY(\tcVar{n}) \nl %
	  & \quad \quad V(k) = phi(Z(k),\tcVar{x}) \nl %
	  & \quad \tcFor{end} \nl %
	  & \quad esp = mean(V) \nl %
